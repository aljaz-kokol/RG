...
template<>
void ShaderProgram::setUniform<int>(std::string_view uniformName, const int& value) const {
    GLint location = getUniformLocation(uniformName);
    glUniform1i(location, value);
}

template<>
void ShaderProgram::setUniform<glm::vec3>(std::string_view uniformName, const glm::vec3& value) const {
    GLint location = getUniformLocation(uniformName);
    glUniform3fv(location, 1,  glm::value_ptr(value));
}

template<>
void ShaderProgram::setUniform<float>(std::string_view uniformName, const float& value) const {
    GLint location = getUniformLocation(uniformName);
