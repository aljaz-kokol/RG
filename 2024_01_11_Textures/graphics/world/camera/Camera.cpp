...
bool Camera::pointingAtPosition(const glm::vec3 &pointPosition) const {
    float threshold = 0.01f;  // Adjust this value as needed
    glm::vec3 directionToPoint = glm::normalize(pointPosition - position);
    float dotProduct = glm::dot(directionToPoint, front);
    return dotProduct > 1.0f - threshold;
}
